...
def test_session5_function_name_had_cap_letter():
    ...
    func_names = inspect.getmembers(session5, inspect.isfunction)
    for name in func_names:
        assert len(re.findall('([A-Z])', name[0])) == 0, "You have used Capital letter(s) in your function names"


############################## Assignment Validations###########################

def test_session5_time_it_print():
    duration = session5.time_it(print, 1, 2, 3, sep='-', end=' ***\n', repetitions=5)
    assert duration > 0, "Function time_it failed for input function - print."

def test_session5_time_it_squared_power_list():
    print(session5.squared_power_list(2))
    duration = session5.time_it(squared_power_list, 2, start=0, end=5, repetitions=5)
    assert duration > 0, "Function time_it failed for input function - squared_power_list"

def test_session5_time_it_polygon_area():
    duration = session5.time_it(polygon_area, 15, sides=3, repetitions=10)
    assert duration > 0, "Function time_it can't time polygon_area function."

def test_session5_time_it_temp_converter():
    duration = session5.time_it(temp_converter, 100, temp_given_in='f', repetitions=100)
    assert duration > 0, "Function time_it can't time temp_converter function."

def test_session5_time_it_speed_converter():
    duration = session5.time_it(speed_converter, 100, dist='km', time='min', repetitions=200)
    assert duration > 0, "Function time_it can't time speed_converter function."
...
def test_session5_polygon_area_length():
    with pytest.raises(TypeError, match=r".*The type of argument length must be integer*"):
        session5.polygon_area('hello')
    with pytest.raises(TypeError, match=r".*The type of argument length must be integer*"):
        session5.polygon_area(1+2j)

def test_session5_polygon_area_sides():
    with pytest.raises(TypeError, match=r".*The type of argument sides must be integer*"):
        session5.polygon_area(1, sides='ten')
    with pytest.raises(TypeError, match=r".*The type of argument sides must be integer*"):
        session5.polygon_area(1, sides=1+2j)

def test_session5_polygon_area_sides_values():
    length = 10
    with pytest.raises(ValueError, match=r".*The value for argument side must be between 3 and 6 inclusive.*"):
        session5.polygon_area(length, sides=0)
    with pytest.raises(ValueError, match=r".*The value for argument side must be between 3 and 6 inclusive.*"):
        session5.polygon_area(length, sides=1)
    with pytest.raises(ValueError, match=r".*The value for argument side must be between 3 and 6 inclusive.*"):
        session5.polygon_area(length, sides=2)
    with pytest.raises(ValueError, match=r".*The value for argument side must be between 3 and 6 inclusive.*"):
        session5.polygon_area(length, sides=7)

def test_session5_polygon_area_length_values():
    with pytest.raises(ValueError, match=r".*The value for argument length can not be negative.*"):
        session5.polygon_area(-5)
...
def test_session5_temp_converter_temp():
    with pytest.raises(TypeError, match=r".*The value of temp should be integer or float value.*"):
        session5.temp_converter('hello')
    with pytest.raises(TypeError, match=r".*The value of temp should be integer or float value.*"):
